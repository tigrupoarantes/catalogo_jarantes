...
for (const item of items) {
  const found = files.find(f => f.startsWith(item.code + '.'));
  if (found) {
    hasImage.push({ ...item, file: found });
  } else {
    missingImage.push(item);
  }
}

console.log(`TOTAL USER ITEMS: ${items.length}`);
console.log(`HAS IMAGE ON DISK: ${hasImage.length}`);
console.log(`MISSING IMAGE ON DISK: ${missingImage.length}`);
console.log('\nMissing Items:');
missingImage.forEach(m => console.log(`${m.code} - ${m.name}`));
