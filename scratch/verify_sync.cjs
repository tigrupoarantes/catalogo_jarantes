...
async function run() {
  console.log(`Verifying ${codes.length} products in database...`);
  ...
  console.log(`Total products found in DB: ${data.length} / ${codes.length}`);
  
  const missingInDB = codes.filter(c => !data.some(d => d.code === c));
  if (missingInDB.length > 0) {
    console.warn("WARNING: The following codes were not found in the DB at all:", missingInDB);
  }
  
  const nullImages = data.filter(d => !d.imageUrl);
  if (nullImages.length > 0) {
    console.error("ERROR: The following codes in the DB still have NULL imageUrl:", nullImages);
  } else {
    console.log("SUCCESS: All products found in DB have valid, non-null imageUrls!");
    console.log("\nVerification sample:");
    data.slice(0, 5).forEach(d => console.log(`- ${d.code} (${d.name}): ${d.imageUrl}`));
  }
}
